...
document.getElementById("regName").addEventListener("input", function () {
  if (!this.value.trim()) return clearError("nameGroup");
...
document.getElementById("regConfirm").addEventListener("input", function () {
  ...
  if (!this.value) return clearError("confirmGroup");
  this.value === p
    ? clearError("confirmGroup")
    : setError("confirmGroup", "confirmError", "Passwords do not match.");
});

document.getElementById("regAddress").addEventListener("input", function () {
  if (!this.value.trim()) return clearError("addressGroup");
...
makeToggle("togglePass", "regPassword");
makeToggle("toggleConfirm", "regConfirm");

async function checkEmailExists(email) {
  try {
    const response = await fetch(
      `${API_URL}/users?email=${encodeURIComponent(email.toLowerCase())}`,
    );
