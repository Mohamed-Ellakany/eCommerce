document.addEventListener("DOMContentLoaded", async function () {
  ...
  const totalAllRev = orders.reduce(
  ...
  document.getElementById("kpi-cancelled").textContent =
  ...
  orders.forEach((o) => {
    ...
    monthRevenue[key] =
      (monthRevenue[key] || 0) + parseFloat(o.totalPrice || 0);
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (ctx) =>
              ` ${ctx.label}: ${ctx.parsed} (${Math.round((ctx.parsed / orders.length) * 100)}%)`,
          },
        },
      },
    },
  ...
  orders.forEach((o) => {
    const s = (o.status || "pending").toLowerCase();
    if (revByStatus[s] !== undefined)
      revByStatus[s] += parseFloat(o.totalPrice || 0);
  });
  ...
  const topSellers = Object.entries(sellerRev)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 8);
  ...
  tbody.innerHTML = recent
    .map((o, i) => {
      const date = parseDate(o.date).toLocaleDateString("en-GB", {
        day: "numeric",
        month: "short",
        year: "numeric",
      });
